...
```mermaid
graph TD
    A["World-Building (Geography, History, Rules)"] --> B["Character Development (Goals, Fears, Voice)"]
    B --> C["Story Arcs (Major Beats & Turning Points)"]
    C --> D["Scene Construction (Dialogue, Action, Pacing)"]
    D --> E["Prose Polishing (via Diff-Edits)"]
